...
scale = False
time_f = pygame.font.Font(os.path.join('fonts', 'DS-DIGI.TTF'), 40)
...
oldsw =[]
def scale(img):
    return pygame.transform.scale(img, (img.get_width() * WIN.get_width() / 800, img.get_height() * WIN.get_height() / 300)) 
...
def scale_all():
    ...
    if s:
        ...
        oldw = weed
        weed = [i * WIN.get_width() / 800 for i in weed]

        
            
    s = not s

    sharks = [scale(i) for i in sharks]
    msharks = [scale(i) for i in msharks]
    seaweed = scale(seaweed)
    ocean = scale(ocean)
    first = False
...
def draw_scaled(weed):
    WIN.blit(seaweed, weed, WIN.get_height() - seaweed.get_height())
def draw_fish(i):
...
def show_done_screen():
    ...
    center = centerx, centery  = 400, 150
    txt = TEXT_FONT.render('Time\'s up!', 15, (255, 255, 255))
    ...
    if True:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.QUIT:
                ...
                raise DoneErr
                break
            if event.type == pygame.VIDEORESIZE:          # Experimental
                scale_all()
